...
<main
	...
	class={`bg-cat-base z-10 flex h-dvh w-dvw flex-col overflow-hidden bg-gradient-to-tr lg:h-[75dvh] lg:w-[70dvw] ${isFullscreen || isMobile ? 'rounded-none' : 'rounded-xl'}`}
...
<style>
	.grid-pattern {
		background-color: var(--color-cat-mantle);
		background-image:
			linear-gradient(to right, var(--color-cat-surface0) 2px, transparent 2px), linear-gradient(to bottom, var(--color-cat-surface0) 2px, transparent 2px);
