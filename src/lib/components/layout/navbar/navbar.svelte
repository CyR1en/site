...
<nav class="overflow-x-auto text-sm select-none md:text-base lg:px-4 lg:py-3">
	<div class="hidden items-center justify-between gap-2 px-2 lg:flex lg:px-0">
		<div class="flex items-center gap-2">
			<div class="bg-cat-overlay2h-4 w-1.5"></div>
			<a class="flex items-center" href="https://github.com/cyr1en/site" target="_blank" rel="norreferrer">
				<svg
					class="mr-1 h-3 w-3"
	...
	<div class="flex items-center justify-between gap-20 overflow-x-auto px-2 py-3 leading-none lg:px-0 lg:py-0">
			...
			{#each navbarMenu as { title, href, key }, i (i)}
				...
				<li class="shrink-0">
					<a
						...
						class={`flex items-center px-2 py-0.5 leading-none transition-all ${isOnCurrentPath ? 'bg-cat-overlay0 text-cat-mantle' : ''}`}
						aria-label={`${title} (Shortcut: ${key})`}
					>
						{parts.before}{#if parts.highlighted}<span class={`${isOnCurrentPath ? 'text-cat-mantle' : 'text-cat-mauve'} transition-all`}>{parts.highlighted}</span
							>{/if}{parts.after}
					</a>
				</li>
			{/each}
		...
		<div class="not-sr-only hidden items-center gap-2 lg:flex">
			<a class="bg-cat-overlay0 shrink-0 px-2 py-0.5 leading-none text-cat-mantle" href="https://github.com/cyr1en" target="_blank" rel="noreferrer">CyR1en</a>
		</div>
	</div>
</nav>
