...
>
	<div class="group absolute top-1/2 hidden -translate-y-1/2 items-center lg:flex">
		<button class="grid h-6 w-6 place-items-center rounded-full" onclick={() => window.close()} aria-label="Close">
			<div class="h-3 w-3 rounded-full bg-cat-overlay0 transition-colors group-hover:bg-[#f38ba8]"></div>
		</button>
		<button class="grid h-6 w-6 place-items-center rounded-full" aria-label="Minimize">
			<div class="h-3 w-3 rounded-full bg-cat-overlay0 transition-colors group-hover:bg-[#f9e2af]"></div>
		</button>
	...
	<p class="text-cat-surface1 not-sr-only mx-auto hidden font-semibold select-none lg:block">Alacritty</p>
